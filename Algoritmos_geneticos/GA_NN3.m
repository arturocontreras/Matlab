...
%longitud de la cadena de genes, 34 de cadena por num
long_caden=14*(num_neuron_co*(numvars-4+1)+4*(num_neuron_co+1));
long_caden_real=num_neuron_co*(numvars-4+1)+4*(num_neuron_co+1);
generaciones = 100; % iteraciones maxima del algoritmo
n = 50;            % tamaño de la población de soluciones par
probabilidad = 0.02; % probabilidad de que cada gen mute
%err_mut=1;
...
%poblacion
%poblacion=round(rand(n,long_caden));
s = n;
...
% rev
    %% Cruza
    pos_caract1=floor((long_caden-1)*rand(1,num_gen_transf))+1;
...
%    comb4(:,pos_caract2)=poblacion(1:2:end,pos_caract2);
    ...
    %% Mutacion
    s = size(poblacion,1);
    poblacion=[poblacion;abs(poblacion-[(rand(s,long_caden+1)<probabilidad).*repmat([mutacion 0],s,1)])];
    %rev
    %% Cálculo de la aptitud (o viabilidad)
    s = size(poblacion,1);
    for j=1:s
        for k=1:long_caden_real
            poblacion_real(j,k)=polyval(poblacion(j,1+14*(k-1):14*k),2)/10^3;
        end
        poblacion_real(j,long_caden_real+1)=poblacion(j,long_caden+1);
        [W1,W2]=conver_vec_matrs(poblacion_real,j,num_neuron_co,26,4,num_neuron_co+1);
...
for k=1:long_caden_real
    poblacion_real(1,k)=polyval(poblacion(1,1+14*(k-1):14*k),2)/10^3;
end
poblacion_real(1,long_caden_real+1)=poblacion(1,long_caden+1);
[W1,W2]=conver_vec_matrs(poblacion_real,1,num_neuron_co,26,4,num_neuron_co+1);
% Funcion de activacion de la red
n1=layer_nn_logsig(P,W1);
Y2=layer_nn_purelin(n1,W2);
Err1=abs(Ts-Y2);
